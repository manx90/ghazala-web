...
const STATUS_OPTIONS = [
  ...
  { value: SubscriptionStatus.TRIAL, label: 'تجريبي' },
  { value: SubscriptionStatus.PENDING_PAYMENT, label: 'بانتظار الدفع' },
  { value: SubscriptionStatus.PAST_DUE, label: 'متأخر' },
  { value: SubscriptionStatus.CANCELLED, label: 'ملغى' },
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col gap-0.5">
            <span className="font-medium">{row.original.organization?.name ?? '—'}</span>
      ...
      },
      {
        id: 'billingCycle',
        header: 'الدورة',
        cell: ({ row }) => BILLING_CYCLE_LABELS[row.original.billingCycle],
      },
      {
        id: 'expiresAt',
        header: 'ينتهي في',
  ...
  return (
    <PageContainer>
      <div className="flex flex-col gap-6">
        <PageHeader
          title="الاشتراكات"
          description="متابعة اشتراكات المنظمات والإيرادات والمدفوعات العالقة"
          actions={
            <Button
              variant="outline"
              size="sm"
      ...
      <ConfirmDialog
        ...
        title="تفعيل الاشتراك يدوياً"
        description={`تفعيل اشتراك "${activateTarget?.organization?.name}" بدون دفع Whop؟ سيتم تعليم آخر فاتورة معلقة كمدفوعة.`}
        confirmLabel="تفعيل"
        onConfirm={() => {
